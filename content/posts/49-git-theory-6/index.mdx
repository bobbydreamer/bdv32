...
![Github - Push Testing](./git-push.png)  
...
![Github - Pull Testing](./git-pull.png)  
...
1. Doing a fork on the open source project 
2. Once you get the personalized copy that open-source repository to your account, clone it to your local. Remote `origin` is added automatically. 
3. `git remote add` is added to the official open-source repository(upstream). 
    * Basically forked repos have two remotes 
      + Origin : Personalized copy of the forked repo 
      + Upstream : Offical open source repository
...
7. Open a Pull Request(PR) from the new branch to the 'official open-source' repository.
8. PR get reviewed by the maintainer and when all good, gets approved for merge to the main open source project repository. 
...
* **[Branching](../46-git-theory-3)** : Covers checkout, merge, rebase, conflicts
* **[Internals](../47-git-theory-4)** : Architecture
* **[Others](../48-git-theory-5)** : Covers .gitignore, stash, clean
